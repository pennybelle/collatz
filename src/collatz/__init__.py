...
class collatz(dict): # subclass dict is used as cache
    ...
    def __getitem__(self, x):
        value = self.get(x, None) # value (v) is cache dict[x] if not None else dict[x] = None
        if value: return value # if x is in cache return value at key x
    ...
    def calc_seq(self, x, steps=0):
        ...
        if value is None: return self.calc_seq(x, steps) # if x not in cache, recurse  
...
def run_val(start_val, stop_val): # run conjecture in range of i & j
    ...
    start_val = time() # timer
    for x in range(start_val, stop_val+1): cache[x] # fill cache.dict[i through j+1]
    time_taken = time() - start_val # timer results
...
def plot_seq_len(start_val, stop_val):
    cache, time_taken = run_val(start_val, stop_val) # referencing collatz dict to var & fills cache
    ...
    plotter(title, points.keys(), points.values(), x_label, y_label) # plot results on graph
